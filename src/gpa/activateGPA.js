...
addCheckboxes = function () {
  ...
  elems.each(function (i) {
    ...
    grade = $(this).children('.col8').html().trim()
      .slice(6);
    console.log(grade, grade.length);
    if (excludeList.indexOf(type) > -1 || grade == '' || grade == 'I') isChecked = false;
    if (isChecked) coursesChecked.add(courseId);
...
showTotalGpa = function () {
  ...
  totalGrades = 0;
  totalCredits = 0;
  if ($('.cgpa_cal_check').length == 0) addCheckboxes();
  elems = $('.hierarchyLi.dataLi').not('.hierarchyHdr, .hierarchySubHdr');
  ...
  elems.each(function (i) {
    ...
    course.credits = Number($(this).children('.col3').html().trim()
      .slice(6));
    if (!(course.grade in gradeValues)) return;

    if (typeCreditsMap.has(course.type)) typeCreditsMap.set(course.type, typeCreditsMap.get(course.type) + course.credits);
    else typeCreditsMap.set(course.type, course.credits);
  ...
  return {
    ...
    branch: $('.studentInfoDiv>.flexDiv:nth-child(5)').find('div:nth-child(1)').find('span').text()
      .trim(),
    studentType: $('.studentInfoDiv>.flexDiv:nth-child(5)').find('div:nth-child(2)').find('span').text()
      .trim(),
